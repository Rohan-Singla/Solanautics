...
interface WhaleLeaderboardProps {
  onTrackWallet?: (wallet) => void
}

export function WhaleLeaderboard({ onTrackWallet }: WhaleLeaderboardProps) {
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-gray-800 bg-gray-900/80 text-left text-xs font-medium uppercase tracking-wider text-gray-400">
            ...
            <th className="whitespace-nowrap px-4 py-3">
              <button className="flex items-center gap-1" onClick={() => handleSort("address")}>
                Wallet Address
                <ArrowUpDown className="h-3 w-3" />
              </button>
            </th>
            <th className="whitespace-nowrap px-4 py-3">
              <button className="flex items-center gap-1" onClick={() => handleSort("totalVolume")}>
                Total Volume
                <ArrowUpDown className="h-3 w-3" />
              </button>
            </th>
            {!isMobile && (
              <th className="whitespace-nowrap px-4 py-3">
                <button className="flex items-center gap-1" onClick={() => handleSort("estimatedProfit")}>
                  Est. Profit
                  <ArrowUpDown className="h-3 w-3" />
                </button>
              </th>
            ...
            {!isMobile && (
              <th className="whitespace-nowrap px-4 py-3">
                <button className="flex items-center gap-1" onClick={() => handleSort("buySellRatio")}>
                  Buy/Sell Ratio
                  <ArrowUpDown className="h-3 w-3" />
                </button>
              </th>
            ...
            {!isMobile && (
              <th className="whitespace-nowrap px-4 py-3">
                <button className="flex items-center gap-1" onClick={() => handleSort("lastTradeTime")}>
                  Last Trade
                  <ArrowUpDown className="h-3 w-3" />
                </button>
              </th>
        ...
        <tbody className="divide-y divide-gray-800">
          {whaleData.map((whale, index) => (
            <tr key={whale.id} className="bg-gray-900/30 text-sm text-gray-300 transition-colors hover:bg-gray-800/50">
                  ...
                  <div className="flex w-24 items-center gap-2">
                    <div className="h-1.5 flex-1 rounded-full bg-gray-700">
                      <div
                        ...
                        style={{
                          width: `${Number.parseInt(whale.buySellRatio.split("/")[0])}%`,
                        }}
                      ></div>
                    </div>
              ...
              {!isMobile && <td className="whitespace-nowrap px-4 py-3 text-gray-400">{whale.lastTradeTime}</td>}
                ...
                <div className="flex justify-end gap-2">
                  <Button
                    ...
                    onClick={() => {
                      ...
                      window.open(`https://solscan.io/account/${whale.rawAddress}`, "_blank")
                    }}
                  >
                    View Details
