...
export function SideNav({ activeTab, setActiveTab }: SideNavProps) {
  const navItems = [
    { id: "whaletracking", label: "Whale Tracking", icon: BarChart3 },
      ...
      <nav className="flex-1 space-y-1 p-4">
        {navItems.map((item) => (
          <Link
            ...
            className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${activeTab === item.id
